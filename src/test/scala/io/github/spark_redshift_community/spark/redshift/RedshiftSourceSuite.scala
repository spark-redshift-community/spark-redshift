...
class RedshiftSourceSuite
  ...
  test("Sets hadoop configs if sse_kms_key is specified") {
    val mockRedshift = new MockRedshift(
      defaultParams("url"),
      Map(TableName.parseFromEscaped(defaultParams("dbtable")).toString -> null))
    val source = new DefaultSource(mockRedshift.jdbcWrapper, _ => mockS3Client)
    source.createRelation(testSqlContext, SaveMode.Append, defaultParams + ("sse_kms_key" -> "abc123"), expectedDataDF)
    Map(
      ...
      "fs.s3.server-side-encryption.key" -> "abc123",
      "fs.s3n.server-side-encryption.key" -> "abc123",
      "fs.s3a.server-side-encryption.key" -> "abc123"
    ).foreach {
      case (key, value) => assert(testSqlContext.sparkContext.hadoopConfiguration.get(key) == value)
    }
  }
