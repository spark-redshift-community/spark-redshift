...
class RedshiftReadSuite extends IntegrationSuiteBase {
  ...
  test("backslashes in queries/subqueries are escaped (regression test for #215)") {
    withUnloadFormat {
      () =>
        val loadedDf =
        read.option("query", s"select replace(teststring, '\\\\', '') as col" +
          s" from $test_table").load()
        checkAnswer(
  ...
  test("Can load output of Redshift aggregation queries") {
    withUnloadFormat {
      () =>
        checkAnswer(
          read.option("query", s"select testbool, count(*) from $test_table" +
            s" group by testbool").load(),
  ...
  test("filtering based on date constants (regression test for #152)") {
    withUnloadFormat {
      () =>
        ...
        // This query failed in Spark 1.6.0 but not in earlier versions.
        // It looks like 1.6.0 performs
    ...
    withUnloadFormat {
      () =>
        ...
        try {
          ...
          checkAnswer(
            read.option("dbtable", tableName).load(),
            Seq(Double.NaN, Double.PositiveInfinity, Double.NegativeInfinity)
              .map(x => Row.apply(x)))
        } finally {
          conn.prepareStatement(s"drop table if exists $tableName").executeUpdate()
        }
    ...
    withUnloadFormat {
      () =>
        withTempRedshiftTable("records_with_escaped_characters") { tableName =>
          val tableName = "records_with_escaped_characters"
          conn.createStatement().executeUpdate(
