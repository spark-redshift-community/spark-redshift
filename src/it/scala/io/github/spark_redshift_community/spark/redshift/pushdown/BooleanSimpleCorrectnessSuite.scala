...
abstract class BooleanSimpleCorrectnessSuite extends IntegrationPushdownSuiteBase {
  ...
  test("child startswith pushdown - with quote") {
    checkAnswer(
      ...
      Seq(Row(1)))
    checkSqlStatement(
      s"""SELECT "teststring" FROM $test_table WHERE "teststring" IS NOT NULL""".stripMargin)
  }
...
class DefaultNoPushdownSimpleBooleanCorrectnessSuite extends BooleanSimpleCorrectnessSuite {
  override protected val auto_pushdown: String = "false"
  override protected val s3format: String = "DEFAULT"
}

class ParquetNoPushdownSimpleBooleanCorrectnessSuite extends BooleanSimpleCorrectnessSuite {
  override protected val s3format: String = "PARQUET"
  override protected val auto_pushdown: String = "false"
}