...
abstract class BooleanSimpleCorrectnessSuite extends IntegrationPushdownSuiteBase {
  ...
  test("child EqualNullSafe pushdown") {
    val input = List(
      ("testbyte", 0, 2, ""),
      ("testdouble", 1234152.12312498, 1, ""),
      ("testfloat", 1.0, 1, "::float4"),
    ...
    input.foreach(test_case => {
      ...
      checkSqlStatement(
        s"""SELECT ( COUNT ( 1 ) ) AS "SUBQUERY_2_COL_0"
           |FROM ( SELECT * FROM ( SELECT * FROM $test_table AS "RS_CONNECTOR_QUERY_ALIAS" )
           |AS "SUBQUERY_0" WHERE CASE WHEN
           | ( ( "SUBQUERY_0"."$column_name" IS NULL ) OR ( $match_value $cast IS NULL ) )
           | THEN ( ( "SUBQUERY_0"."$column_name" IS NULL ) AND ( $match_value $cast IS NULL ) )
           | ELSE ( "SUBQUERY_0"."$column_name" = $match_value $cast ) END )
           |AS "SUBQUERY_1" LIMIT 1""".stripMargin)
    })
  }

  test("child EqualNullSafe pushdown (boolean type)") {
    // "Column name",match value and result size
    val input = List(
      ("testbool", true, 1),
    )
    input.foreach(test_case => {
      ...
      checkSqlStatement(
        s"""SELECT ( COUNT ( 1 ) ) AS "SUBQUERY_2_COL_0"
           |FROM ( SELECT * FROM ( SELECT * FROM $test_table AS "RS_CONNECTOR_QUERY_ALIAS" )
           |AS "SUBQUERY_0"
           |WHERE CASE WHEN ( ( "SUBQUERY_0"."$column_name" IS NULL ) OR ( $match_value IS NULL ) )
           |THEN ( ( "SUBQUERY_0"."$column_name" IS NULL ) AND ( $match_value IS NULL ) )
           |ELSE ( "SUBQUERY_0"."$column_name" = $match_value ) END ) AS "SUBQUERY_1" LIMIT 1""".stripMargin
        )
    })
  }

  test("child EqualNullSafe pushdown (double different type)") {
    checkAnswer(
      sqlContext.sql(s"""SELECT count(*) FROM test_table where testdouble <=> 1234152 """),
      Seq(Row(0)))
    checkSqlStatement(
      s"""SELECT ( COUNT ( 1 ) ) AS "SUBQUERY_2_COL_0" FROM (
         |SELECT * FROM ( SELECT * FROM $test_table AS "RS_CONNECTOR_QUERY_ALIAS" )
         |AS "SUBQUERY_0" WHERE CASE WHEN ( ( "SUBQUERY_0"."TESTDOUBLE" IS NULL ) OR ( 1234152.0 IS NULL ) )
         |THEN ( ( "SUBQUERY_0"."TESTDOUBLE" IS NULL ) AND (1234152.0 IS NULL ) )
         |ELSE ( "SUBQUERY_0"."TESTDOUBLE" = 1234152.0 ) END )
         |AS "SUBQUERY_1" LIMIT 1""".stripMargin)
  }

  test("child EqualNullSafe pushdown (int different type)") {
    checkAnswer(
      sqlContext.sql(s"""SELECT count(*) FROM test_table where testint <=> 42.0 """),
      Seq(Row(2)))
    checkSqlStatement(
      s"""SELECT ( COUNT ( 1 ) ) AS "SUBQUERY_2_COL_0" FROM
         |( SELECT * FROM ( SELECT * FROM $test_table AS "RS_CONNECTOR_QUERY_ALIAS" )
         |AS "SUBQUERY_0" WHERE CASE WHEN ( ( "SUBQUERY_0"."TESTINT" IS NULL ) OR ( 42 IS NULL ) )
         |THEN ( ( "SUBQUERY_0"."TESTINT" IS NULL ) AND ( 42 IS NULL ) )
         |ELSE ( "SUBQUERY_0"."TESTINT" = 42 ) END ) AS "SUBQUERY_1" LIMIT 1""".stripMargin)
  }

  test("child EqualNullSafe pushdown (long different type)") {
    checkAnswer(
      sqlContext.sql(s"""SELECT count(*) FROM test_table where testlong <=> 1239012341823719.0 """),
      Seq(Row(4)))
    checkSqlStatement(
      s"""SELECT ( COUNT ( 1 ) ) AS "SUBQUERY_2_COL_0" FROM
         |( SELECT * FROM ( SELECT * FROM $test_table AS "RS_CONNECTOR_QUERY_ALIAS" )
         |AS "SUBQUERY_0" WHERE CASE WHEN ( ( "SUBQUERY_0"."TESTLONG" IS NULL ) OR (1239012341823719 IS NULL ) )
         |THEN ( ( "SUBQUERY_0"."TESTLONG" IS NULL ) AND (1239012341823719 IS NULL ) )
         |ELSE ( "SUBQUERY_0"."TESTLONG" = 1239012341823719 ) END )
         |AS "SUBQUERY_1" LIMIT 1""".stripMargin)
  }

  test("child EqualNullSafe pushdown (date type)") {
    checkAnswer(
      sqlContext.sql(s"""SELECT count(*) FROM test_table where testdate <=> '2015-07-01' """),
      Seq(Row(1)))
    checkSqlStatement(
      s"""SELECT ( COUNT ( 1 ) ) AS "SUBQUERY_2_COL_0"
         |FROM ( SELECT * FROM ( SELECT * FROM $test_table
         AS "RS_CONNECTOR_QUERY_ALIAS" ) AS "SUBQUERY_0"
         |WHERE CASE WHEN ( ( "SUBQUERY_0"."TESTDATE" IS NULL ) OR
         |( DATEADD(day, 16617, TO_DATE(\\'1970-01-01\\', \\'YYYY-MM-DD\\') ) IS NULL) )
         |THEN ( ( "SUBQUERY_0"."TESTDATE" IS NULL ) AND
         |( DATEADD(day, 16617, TO_DATE(\\'1970-01-01\\', \\'YYYY-MM-DD\\') ) IS NULL) )
         |ELSE ( "SUBQUERY_0"."TESTDATE" = DATEADD(day, 16617 ,
         |TO_DATE(\\'1970-01-01\\', \\'YYYY-MM-DD\\')) ) END ) AS "SUBQUERY_1"
         |LIMIT 1""".stripMargin)
  }

  test("child EqualNullSafe pushdown (string type)") {
    checkAnswer(
      sqlContext.sql(s"""SELECT count(*) FROM test_table where teststring <=> 'asdf' """),
      Seq(Row(1)))
    checkSqlStatement(
      s"""SELECT ( COUNT ( 1 ) ) AS "SUBQUERY_2_COL_0"
         |FROM ( SELECT * FROM ( SELECT * FROM $test_table AS "RS_CONNECTOR_QUERY_ALIAS" )
         |AS "SUBQUERY_0" WHERE CASE WHEN ( ( "SUBQUERY_0"."TESTSTRING" IS NULL ) OR (\\'asdf\\'IS NULL) )
         |THEN ( ( "SUBQUERY_0"."TESTSTRING" IS NULL ) AND (\\'asdf\\'IS NULL) )
         |ELSE ( "SUBQUERY_0"."TESTSTRING" = \\'asdf\\' ) END ) AS "SUBQUERY_1"
         |LIMIT 1""".stripMargin)
  }

  test("child EqualNullSafe pushdown (timestamp type)") {
    checkAnswer(
      sqlContext.sql(
        s"""SELECT count(*) FROM test_table
           |where testtimestamp <=> '2015-07-02 00:00:00.000' """.stripMargin),
    ...
    checkSqlStatement(
      s"""SELECT ( COUNT ( 1 ) ) AS "SUBQUERY_2_COL_0"
         |FROM ( SELECT * FROM ( SELECT * FROM $test_table AS "RS_CONNECTOR_QUERY_ALIAS" )
         |AS "SUBQUERY_0" WHERE CASE WHEN ( ( "SUBQUERY_0"."TESTTIMESTAMP" IS NULL )
         |OR (\\'2015-07-02 00:00:00\\' ::TIMESTAMP IS NULL) )
         |THEN ( ( "SUBQUERY_0"."TESTTIMESTAMP" IS NULL ) AND (\\'2015-07-02 00:00:00\\' ::TIMESTAMP IS NULL) )
         |ELSE ( "SUBQUERY_0"."TESTTIMESTAMP" = \\'2015-07-02 00:00:00\\' ::TIMESTAMP ) END )
         |AS "SUBQUERY_1" LIMIT 1""".stripMargin)
  }

  test("child EqualNullSafe pushdown with null values") {
    val input = Seq((1, Seq(Row(true), Row(false), Row(false))),
      (null, Seq(Row(false), Row(true), Row(false))))

    val df = read.option("query",
        """SELECT 1 as a UNION ALL SELECT null as a UNION ALL SELECT 2 as a""").load()

    input.foreach {
      case (matchValue, expectedRows: Seq[Row]) =>
        checkAnswer(
          df.selectExpr(s"a <=> $matchValue"), expectedRows)
        checkSqlStatement(
        s"""SELECT ( CASE WHEN ( ( "SUBQUERY_0"."A" IS NULL ) OR ( $matchValue IS NULL ) )
          |THEN ( ( "SUBQUERY_0"."A" IS NULL ) AND ( $matchValue IS NULL ) )
          |ELSE ( "SUBQUERY_0"."A" = $matchValue ) END ) AS "SUBQUERY_1_COL_0"
          |FROM ( SELECT * FROM ( SELECT 1 as a UNION ALL SELECT null as a UNION ALL SELECT 2 as a )
          |AS "RS_CONNECTOR_QUERY_ALIAS" ) AS "SUBQUERY_0"""".stripMargin,
          // Spark simplifies the <=> operator into an is null check
          // when one operand is a literal null
          """SELECT ( ( "SUBQUERY_0"."A" IS NULL ) ) AS "SUBQUERY_1_COL_0"
            |FROM ( SELECT * FROM ( SELECT 1 as a UNION ALL SELECT null as a UNION ALL SELECT 2 as a )
            |AS "RS_CONNECTOR_QUERY_ALIAS" ) AS "SUBQUERY_0"""".stripMargin
        )
    }
