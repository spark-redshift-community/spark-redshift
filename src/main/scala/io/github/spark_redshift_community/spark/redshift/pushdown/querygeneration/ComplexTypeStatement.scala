...
import io.github.spark_redshift_community.spark.redshift.pushdown.querygeneration.MiscStatement.getCastType
...
private[querygeneration] object ComplexTypeStatement {
  ...
  def unapply(
    ...
    Option(expr match {
      ...
      case GetMapValue(_, _, true) => throw cannotPushdownStrictIndexOperatorExpression(expr)
      ...
      case GetMapValue(child, key: Literal, _) if key.dataType == StringType => {
   ...
   */
  private def cannotPushdownStrictIndexOperatorExpression(expr: Expression)
   ...
   */
  private def handleComplexTypeSubfieldConversion(expression: Expression,
